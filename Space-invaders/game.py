...
window = pyglet.window.Window(
    width=1200, height=800, caption="Space Invaders", resizable=False
)
window.set_location(80, 25)
...
space = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/space.jpg"
)
player_image = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/PlayerShip.png"
)  # space_ship.png

player_laser = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/laser.png"
)
enemy_laser = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/enemy_laser.png"
)
stats_bg_image = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/stats_bg_white.png"
)

# loading sprite sheet animations for the ufo_head
ufo_head = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/ufoHead_Sh.png"
)
...
enemy_ship = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/enemyShip_Sh01.png"
)
enemy_ship_seq = pyglet.image.ImageGrid(
    enemy_ship, 1, 15, item_width=100, item_height=100
)
enemy_ship_texture = pyglet.image.TextureGrid(enemy_ship_seq)


# loading sprite sheet animations for explosion
xplosion_image = pyglet.image.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sprites/explosion.png"
)
xplosion_seq = pyglet.image.ImageGrid(
    xplosion_image, 4, 5, item_width=96, item_height=96
)
xplosion_textures = pyglet.image.TextureGrid(xplosion_seq)
xplosion_anim = pyglet.image.Animation.from_image_sequence(
    xplosion_textures[0:], 0.05, loop=True
)
...
text3 = pyglet.text.Label("HEALTH", x=1000, y=650, batch=main_batch)
text3.italic = True
text3.bold = True
text3.font_size = 16
...
high_score = load_high_score(
    "/home/frangiayetto/escritorio/Space-invaders/res/score.txt"
)
...
explosion = pyglet.media.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sounds/exp_01.wav",
    streaming=False,
)
player_gun_sound = pyglet.media.load(
    "/home/frangiayetto/escritorio/Space-invaders/res/sounds/player_gun.wav",
    streaming=False,
)
...
def enemy_spawn(dt):
    ...
    if player_is_alive:
        if ufo_head_spawner <= 0:
            enemy_list.append(
                pyglet.sprite.Sprite(ufo_head, x=600, y=950, batch=main_batch)
            )
            enemy_list[-1].speed = randint(100, 300) * choice(
                directions
            )  # last spawned entity in entity list
            enemy_list[-1].hit_count = 0
            enemy_list[-1].MAX_HIT = 2
            ufo_head_spawner += ufo_head_spawner_count
        if enemy_ship_spawner <= 0:
            enemy_list.append(
                pyglet.sprite.Sprite(enemy_ship, x=600, y=950, batch=main_batch)
            )
            enemy_list[-1].speed = randint(100, 300) * choice(
                directions
            )  # last spawned entity in entity list
...
def enemy_shoot(dt):
    ...
    if enemy_fire_rate <= 0:
        for enemy in enemy_list:
            if randint(0, 10) >= 5:
                enemy_laser_list.append(
                    pyglet.sprite.Sprite(
                        enemy_laser, enemy.x + 50, enemy.y, batch=main_batch
                    )
                )
        enemy_fire_rate += 5
...
def update_enemy_shoot(dt):
    for lsr in enemy_laser_list:
        ...
        if (
            lsr.y < -50
        ):  # if the lasers y position is belov -100 it gets removed from the laser list
...
def player_shoot(dt):
    ...
    if player_fire_rate <= 0:
        player_laser_list.append(
            pyglet.sprite.Sprite(
                player_laser, player.x + 32, player.y + 96, batch=main_batch
            )
        )
...
def update_player_shoot(dt):
    for lsr in player_laser_list:
        ...
        if (
            lsr.y > 950
        ):  # if the lasers y position is above 950 it gets removed from the laser list
...
def enemy_hit(entity):
    ...
    if entity.hit_count >= entity.MAX_HIT and player_is_alive:
        ...
        explosion_list.append(
            pyglet.sprite.Sprite(
                xplosion_anim, x=entity.x, y=entity.y, batch=main_batch
            )
        )
        enemy_list.remove(
            entity
        )  # remove the enemy from enemy list when gets shot two times
...
def game_over():
    ...
    if score > int(high_score):
        save_high_score(
            "/home/frangiayetto/escritorio/Space-invaders/res/score.txt", score
        )
...
def bullet_collision(entity, bullet_list):
    for lsr in bullet_list:
        if (
            lsr.x < entity.x + entity.width
            and lsr.x + lsr.width > entity.x
            and lsr.y < entity.y + entity.height
            and lsr.height + lsr.y > entity.y
        ):
            bullet_list.remove(
                lsr
            )  # remove the laser from laser list when colliding with an enemy
            return True
